...
public:
    ...
    Vec3f operator / (const float &r) const { return Vec3f(x / r, y / r, z / r); } //overloaded average
...
public:
    Sphere(const Vec3f &c, const float &r, const Vec3f &color, float kd, float ks, float specExp)
...
public:
    ...
    Cylinder(const Vec3f &a, const Vec3f &b, float r, const Vec3f &color, float kd, float ks, float specExp)
        : p1(a), p2(b), radius(r), radius2(r * r) {
        diffuseColor = color;
        Kd = kd;
        Ks = ks;
        specularExponent = specExp;
    ...
    //clyinder intersection method
    bool intersect(const Vec3f &rayOrigin, const Vec3f &rayDir, float &tNear, uint32_t &index, Vec2f &uv) const override {
    Vec3f axis = p2 - p1; //Cylinder axis vector
    Vec3f originToP1 = rayOrigin - p1; //Vector from cylinder base to ray origin
    float axisDotDir = dotProduct(axis, rayDir); //Projection of ray direction onto cylinder axis
    float axisDotOrigin = dotProduct(axis, originToP1); //Projection of originToP1 onto cylinder axis
    float axisLengthSquared = dotProduct(axis, axis); //Squared length of cylinder axis

    //Components perpendicular to the axis
    Vec3f perpDir = rayDir * axisLengthSquared - axis * axisDotDir;
    Vec3f perpOrigin = originToP1 * axisLengthSquared - axis * axisDotOrigin;
    float a = dotProduct(perpDir, perpDir);
    float b = 2 * dotProduct(perpDir, perpOrigin);
    float c = dotProduct(perpOrigin, perpOrigin) - radius2 * axisLengthSquared * axisLengthSquared;
    ...
    //Check if intersection is within the finite cylinder
    float t = t0;
    Vec3f hitPoint = rayOrigin + rayDir * t;
    float projection = dotProduct(hitPoint - p1, axis) / axisLengthSquared;
    ...
    void getSurfaceProperties(const Vec3f &P, const Vec3f &I, const uint32_t &index, const Vec2f &uv, Vec3f &N, Vec2f &st) const override {
        Vec3f ca = p2 - p1;
        Vec3f pa = P - p1;
        float proj = dotProduct(pa, ca) / dotProduct(ca, ca);
        Vec3f closest = p1 + ca * proj;
...
public:
    //center, orientation, and side length
    Box(const Vec3f &c, float length, const Vec3f &ox, const Vec3f &oy, const Vec3f &oz, const Vec3f &color, float kd, float ks, float specExp)
        : center(c), sideLength(length), orientX(ox), orientY(oy), orientZ(oz) {
        diffuseColor = color;
        Kd = kd;
        Ks = ks;
        specularExponent = specExp;
    ...
    bool intersect(const Vec3f &orig, const Vec3f &dir, float &tnear, uint32_t &index, Vec2f &uv) const override {
        //Transform ray to box's local space to align it with the box's axes
        Vec3f relOrig = orig - center;
        Vec3f localOrig(
            dotProduct(relOrig, orientX),
            dotProduct(relOrig, orientY),
            dotProduct(relOrig, orientZ)
        );
        Vec3f localDir(
            dotProduct(dir, orientX),
            dotProduct(dir, orientY),
            dotProduct(dir, orientZ)
        );

        //calculate the intersection with the box's axis-aligned bounding box
        float halfLength = sideLength * 0.5f;
        Vec3f minBound(-halfLength, -halfLength, -halfLength);
        Vec3f maxBound(halfLength, halfLength, halfLength);

        //calculate intersection with the box's AABB
        float tmin = (minBound.x - localOrig.x) / localDir.x;
        float tmax = (maxBound.x - localOrig.x) / localDir.x;
        if (tmin > tmax) std::swap(tmin, tmax);

    
        float tymin = (minBound.y - localOrig.y) / localDir.y;
        float tymax = (maxBound.y - localOrig.y) / localDir.y;
        if (tymin > tymax) std::swap(tymin, tymax);

        //check if the ray is outside the box  for the y-axis
        if ((tmin > tymax) || (tymin > tmax)) return false;
        tmin = std::max(tmin, tymin);
        tmax = std::min(tmax, tymax);

        float tzmin = (minBound.z - localOrig.z) / localDir.z;
        float tzmax = (maxBound.z - localOrig.z) / localDir.z;
        if (tzmin > tzmax) std::swap(tzmin, tzmax);

        //check if the ray is outside the box for the z-axis
        if ((tmin > tzmax) || (tzmin > tmax)) return false;
        tmin = std::max(tmin, tzmin);
        tmax = std::min(tmax, tzmax);

        //check if the ray is outside the box for the x-axis
    ...
    void getSurfaceProperties(const Vec3f &P, const Vec3f &I, const uint32_t &index, const Vec2f &uv, Vec3f &N, Vec2f &st) const override {
        ...
        Vec3f localP = P - center;
        float px = dotProduct(localP, orientX);
        float py = dotProduct(localP, orientY);
        float pz = dotProduct(localP, orientZ);
        float halfLen = sideLength * 0.5f;

        //determine the normal based on the closest face
        if (std::abs(px - halfLen) < 1e-4) N = orientX;
        else if (std::abs(px + halfLen) < 1e-4) N = -orientX;
        else if (std::abs(py - halfLen) < 1e-4) N = orientY;
        else if (std::abs(py + halfLen) < 1e-4) N = -orientY;
        else if (std::abs(pz - halfLen) < 1e-4) N = orientZ;
        else N = -orientZ;
        N = normalize(N);
    }
...
public:
    ...
    {
        float scale = 10;
        float pattern = sqrtf(st.x * st.x + st.y * st.y); 
        pattern = fmodf(pattern * scale, 1); //almost forgot this
        return mix(Vec3f(0.2, 0.2, 0.8), Vec3f(0.9, 0.9, 0.9), pattern);
    }
    ...
    const std::vector<std::unique_ptr<Light>> &lights)
{
    ...
    const int samples = 16; //number of samples per pixel
    for (int j = 0; j < options.height; ++j) {
        for (int i = 0; i < options.width; ++i) {
            Vec3f pixelColor(0); //accumulate color for the pixel
            for (int s = 0; s < samples; ++s) {
                //randomize sample position within the pixel
                float x = (2 * (i + static_cast<float>(rand()) / RAND_MAX) / (float)options.width - 1) * imageAspectRatio * scale;
                float y = (1 - 2 * (j + static_cast<float>(rand()) / RAND_MAX) / (float)options.height) * scale;
                Vec3f dir = normalize(Vec3f(x, y, -1));
                pixelColor += castRay(orig, dir, objects, lights, options, 0);
            }
            *(pix++) = pixelColor / samples; //average the samples
        }
    }
...
int main(int argc, char **argv)
{
    ...
    Sphere *sph1 = new Sphere(Vec3f(-1, 0, -12), 2, Vec3f(0.2, 0.8, 0.2), 0.8, 0.2, 25); 
    sph1->materialType = DIFFUSE_AND_GLOSSY;

    Sphere *sph2 = new Sphere(Vec3f(0.5, -0.5, -8), 1.5, Vec3f(0.0, 0.0, 1.0), 0.8, 0.2, 25);
    sph2->ior = 1.33;
    sph2->materialType = REFLECTION_AND_REFRACTION;

    Box *box = new Box(
    Vec3f(-2, 0.5, -5), //center
    1.0f,               //side length
    Vec3f(1, 0, 0),     //X orientation
    Vec3f(0, 1, 0),     //Y orientation
    Vec3f(0, 0, 1),     //Z orientation
    Vec3f(0.8, 0.3, 0.3), //color
    0.8, 0.2, 25        //material 
);

    box->materialType = DIFFUSE_AND_GLOSSY;

    Cylinder *cyl = new Cylinder(
    Vec3f(1.5, 0, -8),      //point 1
    Vec3f(1.9, 2, -8),      //point 2
    0.5,                  //radius
    Vec3f(0.2, 0.7, 0.7), //color
    0.8, 0.2, 25          //material
    );
    cyl->materialType = DIFFUSE_AND_GLOSSY;
    

    
    objects.push_back(std::unique_ptr<Sphere>(sph1));
    objects.push_back(std::unique_ptr<Sphere>(sph2));
    objects.push_back(std::unique_ptr<Box>(box));
    objects.push_back(std::unique_ptr<Cylinder>(cyl));
    ...
    options.height = 960;
    options.fov = 70;
    options.backgroundColor = Vec3f(0.8, 0.2, 0.2);
    options.maxDepth = 20;
